...
/// Same as `generic_allocation` but with exportable memory fd on Windows.
#[cfg(feature = "win32")]
...
fn generit_allocation_with_exportable_handle(
    ...
    match pools.entry((memory_type.id(), layout, map)) {
        Entry::Occupied(entry) => match entry.get() {
            &Pool::HostVisible(ref pool) => {
                let alloc =
                    StdHostVisibleMemoryTypePool::alloc_with_exportable_handle(&pool, size, alignment)?;
                let inner = StdMemoryPoolAllocInner::HostVisible(alloc);
                Ok(StdMemoryPoolAlloc {
                    inner,
                    pool: mem_pool.clone(),
            ...
            &Pool::NonHostVisible(ref pool) => {
                let alloc = StdNonHostVisibleMemoryTypePool::alloc_with_exportable_handle(
                    &pool, size, alignment,
                )?;
                let inner = StdMemoryPoolAllocInner::NonHostVisible(alloc);
                Ok(StdMemoryPoolAlloc {
...
unsafe impl MemoryPool for Arc<StdMemoryPool> {
    ...
    fn alloc_generic_with_exportable_handle(
        ...
        generit_allocation_with_exportable_handle(
            self.clone(),
            memory_type,
            size,
            alignment,
